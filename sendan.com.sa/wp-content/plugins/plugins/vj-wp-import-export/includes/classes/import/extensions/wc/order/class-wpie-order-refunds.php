...
class WPIE_Order_Refunds extends \wpie\import\base\WPIE_Import_Base {
    ...
    private function prepare_refunds() {
        ...
        if (!empty($refund_amount)) {
            ...
            $refund_date = !empty($refund_reason) ? explode($delimiter, $refund_date) : array();

            $refund_post_name = wpie_sanitize_field($this->get_field_value('wpie_item_order_item_refund_date'));
            ...
            foreach ($refund_amount as $_key => $_amount) {
                ...
                $args = array(
                    ...
                    'refund_id' => $refund_id,
                    'line_items' => array(),
                    'date_created' => isset($refund_date[$_key]) ? $refund_date[$_key] : ""
                );
                ...
                if ($refund instanceOf \WC_Order_Refund) {
                    ...
                    if ($customer_id) {
                        ...
                        wp_update_post(array('ID' => $refund->get_id(), 'post_author' => $customer_id));

                        update_post_meta($refund->get_id(), '_refunded_by', $customer_id);
                    } else {

                        wp_update_post(array('ID' => $refund->get_id(), 'post_author' => 0));
