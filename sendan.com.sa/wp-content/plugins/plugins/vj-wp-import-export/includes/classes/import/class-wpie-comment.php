...
class WPIE_Comment extends \wpie\import\engine\WPIE_Import_Engine {
    ...
    public function process_import_data() {
        ...
        if ($this->is_update_field("date")) {
            $this->wpie_final_data['comment_date'] = wpie_sanitize_field($this->get_field_value('wpie_item_comment_date'));
        }
        ...
        if ($this->is_new_item) {
            ...
            if ($is_success == 0) {
                ...
                $this->set_log("<strong>" . __('ERROR', 'vj-wp-import-export') . '</strong> : ' . __('Fail to Update comment', 'vj-wp-import-export'));
