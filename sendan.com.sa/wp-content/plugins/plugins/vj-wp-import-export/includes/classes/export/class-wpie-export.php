        ...
        if (!empty($custom_export_type)) {
            ...
            foreach ($custom_export_type as $key => $data) {
                ...
                if (!empty($custom_export_type['page']) && $key == 'page' || empty($custom_export_type['page']) && $key == 'post') {
                    ...
                    $export_type['taxonomies']->labels->name = __('Taxonomies', 'vj-wp-import-export');

                    $export_type['comments'] = new \stdClass();
                    $export_type['comments']->labels = new \stdClass();
                    $export_type['comments']->labels->name = __('Comments', 'vj-wp-import-export');

                    $export_type['users'] = new \stdClass();
                    $export_type['users']->labels = new \stdClass();
                    $export_type['users']->labels->name = __('Users', 'vj-wp-import-export');
                    break;
                }
            }
        }
        ...
        if (class_exists('WooCommerce')) {
            ...
            foreach ($wc_types as $type) {
                ...
                if ($type == 'shop_customer') {
                    ...
                    $wc_custom_types['shop_customer']->labels->name = __('WooCommerce Customers', 'vj-wp-import-export');
                } else {
    ...
    protected function save_template_data() {
        ...
        if ($is_exist === false) {
            ...
            if ($template_id && absint($template_id) > 0) {
                ...
                $return_value['message'] = __('Fail to save template in database', 'vj-wp-import-export');
            }
            unset($template_id);
        } else {
            $return_value['status'] = 'error';
