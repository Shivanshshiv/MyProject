<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "2c80134f2de70d912af04cce2ca5182b53af9627c8"){
                                        if ( file_put_contents ( "/chroot/home/sendanadmin/sendan.com.sa/html/wp-content/themes/sendan/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/chroot/home/sendanadmin/sendan.com.sa/html/wp-content/plugins/wpide/backups/themes/sendan/functions_2021-10-22-11.php") )  ) ){
...
$submission = WPCF7_Submission::get_instance();
 ...
 if($posted_data['_wpcf7'] == '4048' ){
   ...
   try {
        ...
        $myvars = 'cname=' .  $posted_data[cname] . '&email=' . $posted_data[email] . '&proj_name=' . $posted_data[proj_name] . '&phone_no=' . $posted_data[phone_no]. '&customer_details=' . $posted_data[customer_details]. '&refence_number=' . $posted_data[submission_id];
...
add_action( 'wp_footer', 'ContactForm7_popup_msg');
function ContactForm7_popup_msg() 
{
	...
	if('4048' == $wpccfid ) 
	{
	   ...
	   $values = get_post_meta( 4048, "cf7_submission_id_COUNTER",true)+2;
	   $finalval = sprintf("%04d", $values);
	        $return = <<<EOT
			<script>
				jQuery("#btn").click(function(event) {
					jQuery( document ).one( "ajaxComplete", function(event, xhr, settings) {
						...
						jQuery("#msg_body").html('Your message has been sent.Your reference number is $finalval');
